  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="animate-pulse text-gray-500 text-lg font-medium">Loading refunds...</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Refunded Payments</h1>
          <p className="text-sm text-gray-600 mt-1">
            Total Refunded Transactions: {refunds.length}
          </p>
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50 text-gray-600">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider">Gateway</th>
              ...
              {refunds.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-6 py-12 text-center text-gray-500 font-medium">
                    No refunded payments found in the system.
                  </td>
                </tr>
              ) : (
                refunds.map((p, index) => (
                  <tr key={p._id} className="hover:bg-purple-50 transition-colors duration-200">
                    <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">
                      {index + 1}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap font-mono text-xs text-purple-700 font-bold">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-gray-600 capitalize">
                      {p.paymentGateway} <span className="text-[10px] text-gray-400">({p.paymentMethod})</span>
                    </td>
