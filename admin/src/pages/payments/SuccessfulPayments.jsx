  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="animate-pulse text-gray-500 text-lg font-medium">Loading successful payments...</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Successful Payments</h1>
          <p className="text-sm text-gray-600 mt-1">
            Total Confirmed Transactions: {success.length}
          </p>
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {success.length === 0 ? (
                ...
                success.map((p, index) => (
                  <tr key={p._id} className="hover:bg-green-50 transition-colors duration-200">
                    <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">
                      {index + 1}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap font-mono text-xs text-green-700 font-bold">
