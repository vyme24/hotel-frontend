      ...
      <div className="space-y-0.5 px-1.5">
        {[
          ...
          <Link
            key={i}
            to={item.path}
            className={`flex items-center justify-between px-3.5 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all group ${location.pathname === item.path ? 'bg-blue-50 dark:bg-blue-900/10 text-blue-600' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5 hover:text-blue-600'}`}
            onClick={() => setIsProfileOpen(false)}
          >
            <div className="flex items-center gap-2.5">
              <item.icon size={14} className={`${location.pathname === item.path ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600'} transition-colors`} />
              <span>{item.label}</span>
            </div>
            {item.badge > 0 && (
              <span className="w-5 h-5 bg-blue-600 text-white text-[9px] font-black rounded-lg flex items-center justify-center">{item.badge}</span>
            )}
          </Link>
        ))}
      </div>
            ...
            {navigationLinks
              ...
              .map((link) => (
                <div key={link.label} className="relative group/nav">
                  ...
                  {link.dropdown && (
                    <div className="absolute top-full left-0 mt-2 w-56 bg-white dark:bg-[#111] rounded-2xl shadow-[0_30px_60px_rgba(0,0,0,0.15)] dark:shadow-[0_30px_60px_rgba(0,0,0,0.5)] border border-gray-100 dark:border-white/5 py-4 invisible group-hover/nav:visible opacity-0 group-hover/nav:opacity-100 transition-all duration-300 z-[110]">
                      {link.dropdown.map((sub, idx) => (
                        <Link
                          key={idx}
                          to={sub.path}
                          className="block px-6 py-2.5 text-[9px] font-bold uppercase tracking-widest text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all"
                        >
                          {sub.label}
                        </Link>
                      ))}
        ...
        <div className={`absolute right-0 top-0 bottom-0 w-[80%] max-w-sm bg-white dark:bg-[#0a0a0a] shadow-2xl transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
          <div className="p-8 h-full flex flex-col overflow-y-auto">
            <div className="flex items-center justify-between mb-12">
              <span className="text-xl font-black tracking-tighter text-gray-900 dark:text-white uppercase"><span className="text-blue-600">MAKE</span><span className="font-light">ASITE</span></span>
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2.5 rounded-xl bg-gray-50 dark:bg-white/5 text-gray-500 hover:text-blue-600 transition-colors"><X size={20} /></button>
            </div>
                ...
                .map((link) => (
                  <div key={link.label} className="space-y-2">
                    <Link
                      ...
                      className={`block py-3 text-sm font-bold uppercase tracking-widest transition-colors ${isActiveRoute(link.path) ? "text-blue-600" : "text-gray-500 hover:text-blue-600"}`}
                    >
                      {link.label}
                    </Link>
                    {link.dropdown && (
                      <div className="pl-4 space-y-2 border-l border-gray-100 dark:border-white/5">
                        {link.dropdown.map((sub, idx) => (
                          <Link
                            ...
                            className="block py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-blue-600 transition-colors"
                          >
                            {sub.label}
                          </Link>
                        ))}
            ...
            {!isAuthenticated && (
              <div className="pt-8 border-t border-gray-50 dark:border-white/5 space-y-3">
                <button onClick={() => { openModal("login"); setIsMobileMenuOpen(false); }} className="w-full py-4 text-[10px] font-bold uppercase tracking-widest text-gray-500 border border-gray-100 dark:border-white/5 rounded-2xl hover:border-blue-600/30 hover:text-blue-600 transition-all">Sign In</button>
                <button onClick={() => { openModal("register"); setIsMobileMenuOpen(false); }} className="w-full py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-xl shadow-blue-600/30">Register Account</button>
              </div>
            )}
            {isAuthenticated && (
              <div className="pt-8 border-t border-gray-50 dark:border-white/5">
                <button onClick={handleLogout} className="w-full py-4 bg-gray-900 dark:bg-white/10 text-white dark:text-gray-300 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all">Sign Out</button>
              </div>
            )}
          </div>
        </div>
